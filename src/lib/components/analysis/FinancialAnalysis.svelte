<script lang="ts">
	...
	// Service imports are okay (assuming they don't have hidden store dependencies)
	import {
	...
	let anomalies: any = { anomalies: [] }; // Initialize anomalies array
	let predictions: any = null;
	let loading = false; // Local loading for this component's analysis
	let activeTab = 'overview';
	let llmAvailable = false;

	// Check if LLM is available (service call)
	onMount(async () => {
		llmAvailable = await isLLMAvailable();
	});

	// --- Update reactive block to watch appStore.transactions ---
	$: if ($appStore.transactions.length > 0) {
		updateAnalyses();
	} else {
		// Reset local state when transactions are cleared
		summary = null;
		anomalies = { anomalies: [] };
		predictions = null;
	}
	...
	async function updateAnalyses() {
		// Read transactions directly from appStore state
		const currentTransactions = $appStore.transactions;
		if (currentTransactions.length === 0) return;

		loading = true; // Set local loading state

		try {
			// Pass the current transaction array to services
			summary = await calculateFinancialSummary(currentTransactions);

			if (llmAvailable) {
				anomalies = await detectAnomalies(currentTransactions);
				predictions = await predictFutureTransactions(currentTransactions);
			}
		} catch (error) {
			console.error('Error updating analyses:', error);
			// Consider setting an error state for the UI
		} finally {
			loading = false; // Clear local loading state
		}
	}
								...
								<div class="anomaly-card risk-{anomaly.risk}">
									<div class="anomaly-header">
										...
										<span class="anomaly-transaction">
											Transaction #{anomaly.index + 1}: {$appStore.transactions[anomaly.index]
