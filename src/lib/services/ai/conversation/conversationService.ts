...
import { llmChat, DeepSeekApiError } from '../deepseek-client';
...
function handleProcessingError(error: unknown): string {
	...
	if (error instanceof DeepSeekApiError) {
		if (error.status === 401 || error.message.includes('Authentication')) {
			message =
				"Can't connect to AI services (authentication issue). Please check your API key configuration.";
...
export async function sendMessage(message: string): Promise<void> {
	...
	try {
		...
		if (!handled) {
			console.error(
				`[sendMessage] Message fell through ALL handlers unexpectedly: "${message}". This indicates a logic error in the handler chain.`
			);
			// Provide a generic error response
			assistantResponse =
				'Sorry, I encountered an unexpected issue processing that message. Please try rephrasing.';
...
export async function generateSummary(): Promise<void> {
	...
	try {
		...
		summaryResponse = await llmChat(messages, { temperature: 0.5 });
