        ...
        try {
            ...
            for (let batchIndex = 0; batchIndex < batchCount; batchIndex++) {
                ...
                const batchPromises = currentBatchChunks.map(async (chunk, idx) => {
                    ...
                    try {
                        ...
                        console.error(`[BulkTask ${batchId}] Error processing chunk ${chunkIndex}:`, chunkError);
            ...
            if (uniqueTransactionsFromThisRun.length > 0) {
                ...
                if (chunkProcessingErrorOccurred) {
                    finalMessage += 'Note: Some parts of your data might have caused errors during processing.\n\n';
                }
            ...
            if (!currentState._internal.waitingForDirectionClarification && currentState.status !== 'Error processing bulk data' && currentState.status !== 'Error') {
