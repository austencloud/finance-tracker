...
export function getSystemPrompt(todayDateString: string): string {
	...
	// No changes needed to the system prompt for this fix.
...
          "type": "unknown",
          "amount": 20.00,
          "direction": "OUT" // Clear keyword "spent"
        },
        {
...
          "amount": 50.00,
          "direction": "IN" // Clear keyword "received"
        }
      ]
    }
    ...
    Output JSON:
     {
       "transactions": [
         {
           "date": "2025-04-10",
           "description": "Transaction", // Generic description
           "details": "",
           "type": "unknown",
           "amount": 50.00,
           "direction": "unknown" // Direction is ambiguous
         }
       ]
     }
...
        },
         {
           "date": "2025-04-10",
           "description": "MISC TRANSACTION XYZ",
           "details": "",
           "type": "unknown",
           "amount": 25.00,
           "direction": "unknown" // No clear indicator
         }
      ]
    }
...
export function getSummaryPrompt(transactions: Transaction[]): string {
	...
	// No changes needed here for the fix.
...
    ${transactions
		.map((t, i) => {
			...
			// ** Display 'unknown' if direction is unknown **
			const directionDisplay =
				t.direction === 'in' ? 'received' : t.direction === 'out' ? 'spent' : '(direction?)';
			return `${i + 1}. ${t.date !== 'unknown' ? t.date : 'Date unknown'}: ${formatCurrency(amtNum)} ${directionDisplay} ${t.description !== 'unknown' ? 'for ' + t.description : ''} (${t.category})`; // Added category
		})
		.join('\n    ')}
