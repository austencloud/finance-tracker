...
import { llmChat } from './ai/llm'; // Changed from deepseek-client
// --- END UPDATE ---
...
import { llmChunkTransactions } from './ai/conversation/bulk/llm-chunking'; // Note: llmChunkTransactions itself needs internal refactoring if it calls LLM
...
export async function processBulkTransactions(
    ...
    try {
        ...
        if (!chunks || chunks.length === 0) {
            console.error('[processBulkTransactions] No chunks identified in text by llmChunkTransactions.');
        ...
        for (let batchIndex = 0; batchIndex < batchCount; batchIndex++) {
            ...
            console.log(`[processBulkTransactions] Processing chunk batch ${batchIndex + 1}/${batchCount}...`);
            ...
            const batchPromises = currentBatchChunks.map(async (chunk, idx) => {
                ...
                try {
                    ...
                    const aiResponse = await llmChat(messages, { temperature: 0.1 });
                    // --- END UPDATE ---
                    ...
                    if (!parsedTransactions || !Array.isArray(parsedTransactions)) {
                         console.warn(`[processBulkTransactions] Chunk ${chunkIndex}: Failed to parse valid transaction array from AI response.`);
                         // Check if it was likely text
                         if (aiResponse && typeof aiResponse === 'string' && !aiResponse.trim().startsWith('{') && !aiResponse.trim().startsWith('[')) {
                              appStore.updateBulkChunkStatus(chunkIndex, 'error', 'AI returned text, not structured data.');
                         } else {
                              appStore.updateBulkChunkStatus(chunkIndex, 'error', 'Failed to parse AI response structure.');
                         }
