...
async function processInitialData(text: string): Promise<void> {
	...
	try {
		...
		console.log('[processInitialData] extractTransactionsFromTextInternal raw result:', transactions);
		...
		if (transactions && Array.isArray(transactions) && transactions.length > 0) {
            ...
            console.log('[processInitialData] Is transaction list incomplete?', isIncomplete);
             ...
             if (isIncomplete) {
                 ...
                 console.log('[processInitialData] Calling sendMessagesToLLM for *incomplete* transaction clarification...');
                 assistantResponse = await sendMessagesToLLM(apiMessages);
                 console.log('[processInitialData] Incomplete clarification response received:', assistantResponse);
             } else {
            ...
            if (looksLikeTxn) {
                 ...
                 console.log('[processInitialData] Calling sendMessagesToLLM for FAILED extraction clarification...');
                 assistantResponse = await sendMessagesToLLM(apiMessages);
                 console.log('[processInitialData] Failed extraction clarification response received:', assistantResponse);
            } else {
                 ...
                 const apiMessages = [
                     ...
                     { role: 'system', content: getSystemPrompt(today) },
                     ...visibleMessages,
                     { role: 'system', content: "The user's message didn't seem to contain transaction data. Respond conversationally and ask how you can help with their transactions. Do not repeat the initial greeting." }
                 ];
            ...
            if (!assistantResponse || assistantResponse.toLowerCase().includes("hello! i'm your ai transaction assistant")) {
		...
		conversationMessages.update((msgs) => [...msgs, { role: 'assistant', content: 'I encountered an issue while analyzing that data...' }]);
...
export async function sendUserMessage(message: string): Promise<void> {
	...
	if (!initialPromptSent) { // Use internal variable if not using store
...
export async function showTransactionSummary(): Promise<void> {
	...
	try {
		...
		conversationMessages.update((msgs) => [...msgs, { role: 'assistant', content: fallbackMessage }]);
...
function userInputLooksLikeTransaction(text: string): boolean {
    ...
    const hasAmount = /\$\s?\d{1,3}(?:,\d{3})*(?:\.\d{2})?|\d{1,3}(?:,\d{3})*(?:\.\d{2})?\s*(?:dollars|usd|cad|eur|gbp)/i.test(lowerText) || /\b\d+\b/.test(lowerText);
    const hasKeyword = /\b(spent|paid|bought|sold|received|deposit|income|expense|cost|got|transfer|sent|charge|fee|payment|salary|invoice|refund)\b/i.test(lowerText);
    const hasDate = /\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b|\d{1,2}[\/-]\d{1,2}(?:[\/-]\d{2,4})?|\d{4}|\b(yesterday|today|last week|last month|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i.test(lowerText);
